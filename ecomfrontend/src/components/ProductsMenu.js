...
import { getProducts, getSearchedProducts, setCategory,  } from '../store/productRelated/productHandle';
...
const ProductsMenu = ({ dropName }) => {
    ...
    const uniqueItems = [...new Set(productData?.map(product => product.category))];
    useEffect(() => {
                ...
                dropName === "Categories" &&
                    ...
                    >
                        {categoriesList?.map((category,index) => (
                            <MenuItem onClick={() => { catHandler(category) }} key={index}>
                                {category}
                            </MenuItem>
                        ))}
                    </StyledMenu>
