...
import { addUpdateShippingData, getShippingDataIfAvailable } from '../store/productRelated/productHandle';

const ShippingPage = ({ handleNext, profile }) => {
  const { currentUser } = useSelector(state => state.user);
  const {gettedShippingData} = useSelector(state => state.product);
  const dispatch = useDispatch();
  const navigate = useNavigate()
  useEffect(() => {
    if (!gettedShippingData) {
      dispatch(getShippingDataIfAvailable(currentUser?._id));
    }
    console.log(gettedShippingData);
  }, [gettedShippingData, currentUser]);
...
//   let shippingData = currentUser?.shippingData;
  ...
  const [customerId,setCustomerId] = useState(currentUser?._id);
  ...
  const updateShippingData = (shippingData) => {
    ...
    dispatch(addUpdateShippingData(shippingData));
  };
  ...
  const handleSubmit = () => {
    if (validateInputs()) {
      ...
      updateShippingData(updatedFormData);
      handleNext();
    }
  };
  ...
  const profileSubmitHandler = () => {
    if (validateInputs()) {
      console.log("profileSubmitHandler");
      const updatedFormData ={
        ...formData,
        customer: customerId,
      }; 
  ...
  const editHandler = (event) => {
    ...
    if (isNaN(pinCode) || pinCode.length !== 6) {
      ...
      const fields = { address, city, state, country, pinCode, phoneNo,customerId }
      console.log(fields);
  ...
  return (
    <React.Fragment>
      {
        (shippingData && Object.keys(shippingData).length > 0 && changing) ? 
          <React.Fragment>
            ...
            {profile ?
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <GreenButton
                  // onClick={() => navigate("/checkoutsteps")}
